 ...
 */
class Equipment_ListView_Model extends Vtiger_ListView_Model {
	public function getListViewEntries($pagingModel) {
		...
		if (!empty($orderBy) && $orderByFieldModel) {
			...
			if ($popupEntryFromSEMOdal == true) {
				...
				$functionalLocations = getAllAssociatedFunctionalLocationsSE('36x' . $popupEntryFromSEMOdalsourceRecord);
				if (empty($functionalLocations)) {
					$listQuery = $listQuery . ' and 1 = 2 ';
				} else {
					$listQuery = $listQuery . ' AND vtiger_equipment.functional_loc IN ("' . implode('","', $functionalLocations) . '")';
				}
			} else {
				global $current_user;
				$data = getUserDetailsBasedOnEmployeeModuleG($current_user->user_name);
				if (empty($data)) {
					...
					|| $data['cust_role'] == 'Divisonal Service Support')) {
						...
						if (empty($functionalLocations)) {
							...
							$listQuery = $listQuery . ' AND vtiger_equipment.equipmentid IN ("' . implode('","', $functionalLocations) . '")';
						}
					}
				}
			}
