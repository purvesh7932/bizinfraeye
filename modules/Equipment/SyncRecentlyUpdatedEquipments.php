<?php
function SyncRecentlyUpdatedEquipments() {
	global $adb;
	...
	if ($currentUserModel->isAdminUser()) {
		...
		foreach ($xml as $key => $value) {
			...
			if ($num_rows > 0) {
				...
				EQcheckAndInsertOption($optionValue);
			...
			EQcreateWarrantyRecordIfNOtEits($value);
		}
	} else {
...
function EQcreateWarrantyRecordIfNOtEits($value) {
	global $adb;
...
function EQcheckAndInsertOption($option) {
	global $adb;
