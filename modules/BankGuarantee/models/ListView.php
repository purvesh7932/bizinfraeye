...
class BankGuarantee_ListView_Model extends Vtiger_ListView_Model {

    public function getSumOfEnqAmount() {
        ...
        if ($fetchinFormMobile) {
            $fields = $queryGenerator->getFields();
            foreach ($fields as $value) {
                if ($value == 'id' || $value == 'starred') {
                    continue;
                }
        ...
        if ($position) {
            ...
            $listQuery = "SELECT sum($baseTable.bg_val) AS amount_sum ";
        ...
        return $db->query_result($listResult, 0, 'amount_sum');
    }
