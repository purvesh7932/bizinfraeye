...
class ServiceReports_FilePreview_View extends Vtiger_IndexAjax_View {
	...
	public function process(Vtiger_Request $request) {
		...
		$modelFileDetails = $recordModel->getFileDetails();

		for ($i = 0; $i < count($modelFileDetails); $i++) {
			if ($modelFileDetails[$i]['attachmentsid'] == $atid) {
				$fileDetails = $modelFileDetails[$i];
				break;
			}
		}
		$fileContent = false;
		...
		if (empty($downloadUrl)) {
			$downloadUrl =   $url = \Vtiger_Functions::getFilePublicURL($fileDetails['attachmentsid'], $fileDetails['name']);;
		}
