...
class ReturnSaleOrders_Record_Model extends Inventory_Record_Model {
    ...
    function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false) {
        ...
        if (in_array($module, $inventoryModules)) {
            ...
            if (($data['office'] == 'Production Division')) {
                    $query = "SELECT
					...
					WHERE vtiger_inventoryproductrel.id=? and vtiger_inventoryproductrel.div_or_ser_center = ? ORDER BY sequence_no";
                    $params = array($focus->id,  $data['production_division']);
                ...
                LEFT JOIN vtiger_service ON vtiger_service.serviceid=vtiger_inventoryproductrel.productid
                WHERE vtiger_inventoryproductrel.id=? and vtiger_inventoryproductrel.div_or_ser_center = ? ORDER BY sequence_no";
                $params = array($focus->id,  $data['service_centre']);
            } else {
                $query = "SELECT
					case when vtiger_products.productid != '' then vtiger_products.productname else vtiger_service.servicename end as productname,
					case when vtiger_products.productid != '' then vtiger_products.product_no else vtiger_service.service_no end as productcode,
        ...
        for ($i = 1; $i <= $num_rows; $i++) {
            ...
            if (!empty($product_Detail[$i]['vendor_name' . $i])) {
                $product_Detail[$i]['vendor_name_Label' . $i] = Vtiger_Functions::getCRMRecordLabel($product_Detail[$i]['vendor_name' . $i]);
            } else {
                $product_Detail[$i]['vendor_name_Label' . $i] = '';
            }
