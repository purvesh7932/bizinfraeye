...
class PDFMaker_Delete_Action extends Vtiger_Save_Action
{
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        if ($request->has('record') && !$request->isEmpty('record')) {
            ...
            $checkRes = $adb->pquery("select module, type from vtiger_pdfmaker where templateid=?", array($templateid));
            ...
            $adb->pquery("UPDATE vtiger_pdfmaker SET deleted = ? WHERE templateid=?", array('1', $templateid));
        }
