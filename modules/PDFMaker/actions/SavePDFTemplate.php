...
class PDFMaker_SavePDFTemplate_Action extends Vtiger_Action_Controller {
    ...
    public function process(Vtiger_Request $request) {
        ...
        $sql4 = "UPDATE vtiger_pdfmaker_settings SET margin_top = ?, margin_bottom = ?, margin_left = ?, margin_right = ?, format = ?, orientation = ?, decimals = ?, decimal_point = ?, thousands_separator = ?, header = ?, footer = ?, encoding = ?, disp_header = ?, disp_footer= ?,sharingtype= ? WHERE templateid = ?";
        $params4 = array( $margin_top, $margin_bottom, $margin_left, $margin_right, $pdf_format, $pdf_orientation,
            $dec_decimals, $dec_point, $dec_thousands, $header, $footer, $encoding, $disp_header, $disp_footer,$sharingtype, $templateid);
