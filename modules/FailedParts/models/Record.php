...
class FailedParts_Record_Model extends Inventory_Record_Model {
    ...
    function getProductsForSo($request) {
        $numOfCurrencyDecimalPlaces = getCurrencyDecimalPlaces();
        $relatedProducts = $this->getAssociatedProductsSO($this->getModuleName(), $this->getEntity() , '', false, $request);
    ...
    function getAssociatedProductsSO($module, $focus, $seid = '', $refModuleName = false, $request) {
        ...
        if (in_array($module, $inventoryModules)) {
            $query = "SELECT
					...
					WHERE id=? and lineitem_id in (" . generateQuestionMarks($selectedIds) . ")  ORDER BY sequence_no";
        ...
        for ($i = 1; $i <= $num_rows; $i++) {
            ...
            $qty = $adb->query_result($result, $i - 1, 'rcvd_qty_validated');
