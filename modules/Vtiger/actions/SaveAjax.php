...
class Vtiger_SaveAjax_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		//update No. of times of extension while seelction bankguaranteeextenstion purvesh
		if ($sourceModule == 'BankGuarantee' && $relatedModule == 'BankGuaranteeExtensions') {
			$query=$adb->pquery("SELECT *  FROM `vtiger_crmentityrel` WHERE `crmid` = " .$sourceRecordId);
			$currentrecord = 1;
			$total = $adb->num_rows($query);
			$totalrecords = $total+$currentrecord;
			$adb->pquery("update vtiger_bankguarantee set no_times_exten=$totalrecords where bankGuaranteeid=".$sourceRecordId);
		}
		...
		//$log->debug("purvesh request".print_r($request, true));
