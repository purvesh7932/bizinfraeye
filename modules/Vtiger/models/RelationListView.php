...
class Vtiger_RelationListView_Model extends Vtiger_Base_Model {
	...
	public function IGgetRelatedEntriesCount(Vtiger_Request $request) {
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$realtedModuleModel = $this->getRelatedModuleModel();
		$relatedModuleName = $realtedModuleModel->getName();
		...
		if (!empty($request->get('search_params'))) {
			$queryGenerator = new EnhancedQueryGenerator($relatedModuleName, $currentUser);
			$transformedSearchParams = Vtiger_Util_Helper::transferListSearchParamsToFilterCondition(
				$request->get('search_params'),
				$realtedModuleModel
			);
			$queryGenerator->parseAdvFilterList($transformedSearchParams, "CustomQueryBreaker");
			$listQueryWithSearchParams = $queryGenerator->getQuery();
			$sqlWithSearchParam = explode("CustomQueryBreaker", $listQueryWithSearchParams);
			$sql = $sqlWithSearchParam[1];
			if (!empty($sql)) {
				$relationQuery = $relationQuery . ' AND ' . $sql;
			}
		}
    ...
    public function getRelatedEntriesREcordIds(Vtiger_Request $request) {
        $currentUser = Users_Record_Model::getCurrentUserModel();
        $realtedModuleModel = $this->getRelatedModuleModel();
        $relatedModuleName = $realtedModuleModel->getName();
        $relationQuery = $this->getRelationQuery();
        ...
        if ($position) {
            ...
            if ($relatedModuleName == 'Calendar') {
                ...
                $relationQuery = 'SELECT vtiger_crmentity.crmid ';
            }
            for ($i = 1; $i < $splitCount; $i++) {
                $relationQuery = $relationQuery . ' FROM ' . $split[$i];
            }
