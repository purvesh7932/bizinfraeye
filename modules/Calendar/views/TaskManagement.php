...
class Calendar_TaskManagement_View extends Vtiger_Index_View {
	...
	public function showManagementView(Vtiger_Request $request) {
		...
		$viewer->assign('CLASS', $this->generateClass($request));
	...
	public function generateClass(Vtiger_Request $request) {
		$moduleName = $request->getModule();
		$module = Vtiger_Module_Model::getInstance($moduleName);
		$field = Vtiger_Field_Model::getInstance('taskpriority', $module);
		$priorities = $field->getPicklistValues();
	...
	public function getClass($priority) {
		$color = '';
		switch ($priority) {
			case 'High'		:	$color = 'btn btn-soft-danger';	break;
			case 'Medium'	:	$color = 'btn btn-soft-success';	break;
			case 'Low'		:	$color = 'btn btn-soft-primary';	break;
			default			:	$color = 'btn btn-soft-primary';
								break;
		}
		return $color;
	}
