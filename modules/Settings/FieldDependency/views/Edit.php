...
class Settings_FieldDependency_Edit_View extends Settings_Vtiger_Index_View {

	public function process(Vtiger_Request $request) {
		$moduleName = $request->getModule();
		$qualifiedModuleName = $request->getModule(false);

		$moduleModelList = Settings_FieldDependency_Module_Model::getPicklistSupportedModules();
		...
		$recordModel = Settings_FieldDependency_Record_Model::getInstance($selectedModule, $sourceField, $targetField);
	...
	public function getDependencyGraph(Vtiger_Request $request) {
		...
		$sourceField = $request->get('sourcefield');
		$targetField = $request->get('targetfield');
		$recordModel = Settings_FieldDependency_Record_Model::getInstance($module, $sourceField, $targetField);
		$valueMapping = $recordModel->getFieldDependency();
		...
		foreach ($describeInfo['fields'] as $key => $value) {
			$targetFieldNew[$value['name']] =  $value['label'];
		}
		$targetPicklistValues = $targetFieldNew;
