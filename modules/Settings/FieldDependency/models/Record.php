...
class Settings_FieldDependency_Record_Model extends Settings_Vtiger_Record_Model {
	...
	public function getRecordLinks() {
		...
		$editLink = array(
			'linkurl' => "javascript:Settings_FieldDependency_Js.triggerEdit(event, '$soureModule', '$sourceField', '$targetField')",
			'linklabel' => 'LBL_EDIT',
		...
		$deleteLink = array(
			'linkurl' => "javascript:Settings_FieldDependency_Js.triggerDelete(event, '$soureModule','$sourceField', '$targetField')",
	...
	public function getFieldDependency() {
		if(empty($this->mapping)) {
			$dependency = Vtiger_DependencyPicklist::getFieldsFitDependency($this->get('sourceModule'), $this->get('sourcefield'), $this->get('targetfield'));
	...
	public function getNonMappedSourcePickListValues() {
		if(empty($this->nonMappedSourcePickListValues)) {
			...
			$dependencyMapping = $this->getFieldDependency();
	...
	public function save($mapping) {
		...
		Vtiger_DependencyPicklist::saveFieldDependencies($this->get('sourceModule'), $dependencyMap);
		return true;
	}

	public function delete() {
		Vtiger_DependencyPicklist::deleteFieldsFiDependencies($this->get('sourceModule'), $this->get('sourcefield'), $this->get('targetfield'));
		return true;
	}
