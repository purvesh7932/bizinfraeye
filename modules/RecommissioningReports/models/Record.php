...
class RecommissioningReports_Record_Model extends Inventory_Record_Model {
    ...
    public function getAssociatedProductsSOEnabled($module, $focus, $seid = '', $refModuleName = false) {
        global $log;
        ...
        if (in_array($module, $inventoryModules)) {
            $query = "SELECT
					...
					WHERE id=? and vtiger_inventoryproductrel.add_for_cre_so = 1 ORDER BY sequence_no";
    ...
    public function getProductsForSO() {
        $numOfCurrencyDecimalPlaces = getCurrencyDecimalPlaces();
        $relatedProducts = $this->getAssociatedProductsSOEnabled($this->getModuleName(), $this->getEntity());
    ...
    public function getAssociatedProductsViewnabled($module, $focus, $seid = '', $refModuleName = false) {
        global $log;
        $log->debug("Entering getAssociatedProducts(" . $module . "," . get_class($focus) . "," . $seid . "='') method ...");
        global $adb;
        $no_of_decimal_places = getCurrencyDecimalPlaces();
    ...
    public function getProductsForView() {
        $numOfCurrencyDecimalPlaces = getCurrencyDecimalPlaces();
        $relatedProducts = $this->getAssociatedProductsViewnabled($this->getModuleName(), $this->getEntity());
        $productsCount = count($relatedProducts);
    ...
    public function getImageDetails() {
        ...
        if ($recordId) {
            $sql = "SELECT vtiger_attachments.*, vtiger_crmentity.setype FROM vtiger_attachments
                ...
                WHERE vtiger_crmentity.setype In ('RecommissioningReports Attachment' , 'RecommissioningReports Image')  AND vtiger_seattachmentsrel.crmid = ?";
    ...
    public function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false) {
        global $log;
        $log->debug("Entering getAssociatedProducts(" . $module . "," . get_class($focus) . "," . $seid . "='') method ...");
        global $adb;
        $output = '';
    ...
    public function getParentRecordRelatedLineItems($parentRecordModel) {
		...
		$productDetails = $this->getAssociatedProducts($parentRecordModel->getModuleName(), $parentRecordModel->getEntity(), $parentRecordModel->getId(), $this->getModuleName());
		$productIdsList = array();
