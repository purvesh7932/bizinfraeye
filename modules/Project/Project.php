...
class Project extends CRMEntity {
	...
	function save_module($module) {
		...
		// $projectype=$_REQUEST['projectype'];

		// $PtQuery = $this->db->pquery("SELECT * from vtiger_inventoryproductrel where id=".$projectype);
		// $PtFieldsCount = $this->db->num_rows($PtQuery);

		// for($i=0; $i<$PtFieldsCount; $i++) {
		// 	$Pt_Productname = $this->db->query_result($PtQuery, $i, 'productname');
	...
	function geCountQuery($module, $usewhere='') {
		$query = "SELECT count(*) ";
