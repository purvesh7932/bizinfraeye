...
class Users extends CRMEntity {
	...
	function saveentity($module, $fileid='') {
		...
		$email = $this->column_fields['email1'];
		$mono = $this->column_fields['phone_mobile'];
		$name = $this->column_fields['last_name'];
		$user_name = $this->column_fields['user_name'];
		if (!empty($user_name)) {
			$adb->pquery(
				"update vtiger_serviceengineer set 
			service_engineer_name= ? ,email=?,
			phone=? where badge_no= ?",
				array($name, $email, $mono, $user_name)
			);
		}
	...
	function uploadAndSaveFile($id,$module,$file_details,$attachmentType='Attachment') {
		...
		if ($uploadingUserImageFormTheApi == true) {
