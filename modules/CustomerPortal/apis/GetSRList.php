...
class CustomerPortal_GetSRList extends CustomerPortal_API_Abstract {
	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($current_user) {
			...
			array_push($activeFields , 'ticket_type');
			...
			$response->addToResult('count', strval($count));
			$moduleModel = Vtiger_Module_Model::getInstance('HelpDesk');
			$counts = $moduleModel->getTicketsByStatusCountsForCustomer('', '', $headerFieldStatusValue, $customerId);
			$response->addToResult('ticketStatusCounts', $counts);
			return $response;
		}
	}
