	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($num_rows > 0) {
			...
			if ($isActive && ($support_end_date >= $current_date || $support_end_date == null )) {
				...
				$contactEmail = $adb->query_result($res, 0, 'email');
				...
				$mailStatus = send_mail($moduleName, $contactEmail , $HELPDESK_SUPPORT_NAME, $HELPDESK_SUPPORT_EMAIL_ID, $subject, $contents, '', '', '', '', '', true);
