...
class Owner_SaveAjaxSavePassword_Action extends Vtiger_IndexAjax_View {
	...
	public function savePassword(Vtiger_Request $request) {
		...
		$serviceEngid = $request->get('ownerid');
		$email = '';
		if (!empty($serviceEngid)) {
			$recordModel = Vtiger_Record_Model::getInstanceById($serviceEngid, 'Owner');
