	...
	*/
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		...
		if($return_module == 'Leads') {
		    $sql1 = 'DELETE FROM campaigns_leads_sendmail WHERE campaignid=? AND leadid=?';
			$this->db->pquery($sql1, array($id, $return_id));
	...
	function save_related_module($module, $crmid, $with_module, $with_crmids, $otherParams = array()) {
		...
		foreach($with_crmids as $with_crmid) {
			if ($with_module == 'Leads') {
				...
				$sql2 = 'INSERT INTO campaigns_leads_sendmail VALUES(?,?,0)';
				$adb->pquery($sql2, array($crmid, $with_crmid));
