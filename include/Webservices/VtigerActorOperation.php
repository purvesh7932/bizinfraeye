...
class VtigerActorOperation extends WebserviceEntityOperation {
	...
	public function __create($elementType,$element){
		...
		if($this->entityTableName == 'vtiger_inventoryproductrel' ){
			array_pop($element);
		}
		global $moduleNameForLine;
		if ($moduleNameForLine == 'FailedParts' || $moduleNameForLine == 'ReturnSaleOrders') {
			global $IGRecordMode;
			$referenceFields = array('vendor_name');
			if($IGRecordMode == 'Edit') {
				global $iGLineItems;
				$editableFields = vglobal('EDITABLEFIELDSLINE');
				$dateFields = $this->meta->getIGDateFields();
				$lineItemDateFields = array_intersect($dateFields,$editableFields);
				$editableElement = [];
				$updatequery = "update {$this->entityTableName} set ";
				foreach ($element as $eleKey => $eleValue) {
					if (in_array($eleKey, $editableFields) && $eleKey != 'lineitem_id' && isset($iGLineItems[intval($element['sequence_no']) - 1][$eleKey])) {
						if (in_array($eleKey, $lineItemDateFields)) {
							if (strpos($eleValue, "/") !== false) {
								$date = new DateTimeField($eleValue);
								$editableElement[$eleKey] = $date::__convertToDBFormat($eleValue, 'dd/mm/yyyy');
								$updatequery .= " $eleKey = ? , ";
							}
						} else if (in_array($eleKey, $referenceFields)) {
							if (strpos($eleValue, "x") !== false) {
								$id = explode('x', $eleValue);
								$editableElement[$eleKey] = $id[1];
								$updatequery .= " $eleKey = ? , ";
							}
						} else {
							$editableElement[$eleKey] = $eleValue;
							$updatequery .= " $eleKey = ? , ";
						}
					}
				}
				...
				array_push($element, $lineItemId);
			} else {
				foreach ($element as $eleKey => $eleValue) {
					if (in_array($eleKey, $referenceFields)) {
						if (strpos($eleValue, "x") !== false) {
							$id = explode('x', $eleValue);
							$element[$eleKey] = $id[1];
						}
	...
	public function create($elementType,$element){
		...
		if (in_array($moduleNameForLine, $inventoryModules)) {
			return $this->retrieve(vtws_getId($this->meta->getEntityId(),$element['parent_id']));
		} else {
	...
	protected function restrictFields($element, $selectedOnly = false){
		...
		if ($IGRecordMode == 'Edit' && ($moduleNameForLine == 'FailedParts' || $moduleNameForLine == 'ReturnSaleOrders')) {
			$newElement['lineitem_id'] = $element['lineitem_id'];
		}
		return $newElement;
	}
	...
	public function retrieve($id){
		...
		if(!$success && ($this->entityTableName != 'vtiger_inventoryproductrel') ){
