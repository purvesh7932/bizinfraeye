...
function isPermitted($module,$actionname,$record_id='')
{
	...
	if(vtlib_isModuleActive($checkModule)){
		...
		if ($module == 'Equipment') {
			$userRole = $_SESSION['authenticated_user_role'];
			require_once('include/utils/GeneralUtils.php');
			$val = isInAllowedInLinekedEquipments($record_id);
			if ($val == true) {
				return "yes";
			} else if (
				$userRole == 'BEML Management'
				|| $userRole == 'BEML Marketing HQ'
				|| $userRole == 'Divisonal Service Support'
			) {
				return "yes";
			}
		} else if ($module == 'BankGuarantee') {
			require_once('include/utils/GeneralUtils.php');
			$val = isInAllowedInBGFunctionalLocation($record_id);
			if ($val == true) {
				return "yes";
			}
		} else if ($module == 'FunctionalLocations') {
			require_once('include/utils/GeneralUtils.php');
			$val = isInAllowedInFuncFunctionalLocation($record_id);
			if ($val == true) {
				return "yes";
			}
		}
