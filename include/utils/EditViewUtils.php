...
function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false)
{
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		if (!empty($product_Detail[$i]['line_vendor_id' . $i])) {
			$product_Detail[$i]['line_vendor_id_Label' . $i] = Vtiger_Functions::getCRMRecordLabel($product_Detail[$i]['line_vendor_id' . $i]);
		} else {
			$product_Detail[$i]['line_vendor_id_Label' . $i] = '';
		}
...
function getAssociatedProductsAnother($module, $focus, $seid = '', $refModuleName = false) {
	...
	for ($i = 1; $i <= $num_rows; $i++) {
		...
		$product_Detail[$i]['comment_other' . $i] = from_html($adb->query_result($result, $i - 1, 'comment_other'));
