...
function deleteEquipmentLinesWithRecordId($recordId , $table) {
	global  $adb;
	$adb->pquery("delete from $table where id=?", array($recordId));
}
...
function saveInventoryProductDetails1(&$focus, $module, $update_prod_stock = 'false', $updateDemand = '') {
	...
	$fieldNames = [];
	$fieldNamesData = [];
	if ($module == 'FailedParts' || $module == 'ServiceReports' || $module == 'RecommissioningReports') {
		$tabId = getTabId($module);
		$sql = "SELECT * FROM `vtiger_field` LEFT JOIN vtiger_blocks
         on vtiger_blocks.blockid = vtiger_field.block where vtiger_field.tabid = ? 
	...
	for ($i = 1; $i <= $tot_no_prod; $i++) {
		...
		$listprice = vtlib_purify($_REQUEST['listPrice' . $i]);
		$comment = vtlib_purify($_REQUEST['comment_other' . $i]);
		$purchaseCost = vtlib_purify($_REQUEST['purchaseCost' . $i]);
		$margin = vtlib_purify($_REQUEST['margin' . $i]);
		...
		if ($module == 'FailedParts' || $module == 'ServiceReports' || $module == 'RecommissioningReports') {
			...
			if (empty($fieldNamesFromOb)) {
				$query = "INSERT INTO vtiger_inventoryproductrel_other(id, productid, productname, sequence_no, quantity, comment_other)
					VALUES(?,?,?,?,?,?)";
			} else {
				$query = "INSERT INTO vtiger_inventoryproductrel_other(id, productid, productname, sequence_no, quantity,comment_other, " . implode(',', $fieldNamesFromOb) . ")
					VALUES(?,?,?,?,?,?, " . generateQuestionMarks($fieldNamesFromOb) . ")";
			}
			$qparams = array($focus->id, $prod_id, $prod_name, $prod_seq, $qty, $comment);
...
function saveInventoryProductDetailsFromMobile($LineItemsOther, $module, $createdId, $recordValues) {
	...
	foreach ($LineItemsOther as $lineItem) {
		...
		if (empty($fieldNames)) {
			...
			$query = "INSERT INTO vtiger_inventoryproductrel_other(id, productid, productname, sequence_no, quantity,comment_other, " . implode(',', $fieldNames) . ")
					VALUES(?,?,?,?,?,?, " . generateQuestionMarks($fieldNames) . ")";
		}
		$qparams = array($createdId, $prod_id, $prod_name, $prod_seq, $qty, $comment);
		$qparams = array_merge($qparams, $fieldNameValues);
		$adb->pquery($query, $qparams);
		$prod_seq = $prod_seq + 1;
	}
