...
// if (vtigercron_detect_run_in_cli() || (isset($_SESSION["authenticated_user_id"]) &&	isset($_SESSION["app_unique_key"]) && $_SESSION["app_unique_key"] == $application_unique_key)) {
	...
	include_once('modules/StockTransferOrders/StockTransferOrdersExternalAppSyncUpdated.php');
	StockTransferOrdersExternalAppSyncUpdated([]);
	include_once('modules/ServiceOrders/ExternalAppSyncUpdated.php');
	ExternalAppSyncUpdated([]);
	include_once('modules/Equipment/SyncRecentlyUpdatedEquipments.php');
	SyncRecentlyUpdatedEquipments();
// } else {
// 	echo ("Access denied!");
// }
