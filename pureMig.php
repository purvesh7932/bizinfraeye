...
if ($blockInstance) {
    $fieldInstance = Vtiger_Field::getInstance('serv_eng_pre_status', $moduleInstance);
    if (!$fieldInstance) {
        $fieldInstance = new Vtiger_Field();
        $fieldInstance->name = 'serv_eng_pre_status';
        $fieldInstance->label = 'Service Engineer Status';
        $fieldInstance->table = $moduleInstance->basetable;
        $fieldInstance->column = 'serv_eng_pre_status';
        $fieldInstance->uitype = '16';
        $fieldInstance->presence = '0';
        $fieldInstance->typeofdata = 'V~O';
        $fieldInstance->columntype = 'VARCHAR(100)';
        $fieldInstance->defaultvalue = NULL;
        $blockInstance->addField($fieldInstance);
        $fieldInstance->setPicklistValues(array('Present', 'Absent', 'Leave'));
    } else {
        echo "field is already Present --- serv_eng_pre_status in ServiceEngineer Module --- <br>";
    }
} else {
...
if ($blockInstance) {
    $fieldInstance = Vtiger_Field::getInstance('mode_of_payment', $invoiceModule);
    if (!$fieldInstance) {
        $field = new Vtiger_Field();
        $field->name = 'mode_of_payment';
        $field->column = 'mode_of_payment';
        $field->table = $invoiceModule->basetable;
        ...
        $field->displaytype = 1;
        $blockInstance->addField($field);
        $field->setPicklistValues(array('Online', 'Cash'));
    } else {
        echo "field is present -- mode_of_payment In HelpDesk Module --- <br>";
    }
} else {
...
$blockInstance = Vtiger_Block::getInstance('LBL_TICKET_INFORMATION', $invoiceModule);
if ($blockInstance) {
    $fieldInstance = Vtiger_Field::getInstance('new_parts_instled', $invoiceModule);
    if (!$fieldInstance) {
        $field = new Vtiger_Field();
        $field->name = 'new_parts_instled';
        $field->column = 'new_parts_instled';
        $field->table = $invoiceModule->basetable;
        $field->label = 'New Parts Installed';
        $field->uitype = '999';
        $field->columntype = 'VARCHAR(100)';
        $field->typeofdata = 'V~O';
        $field->displaytype = 1;
        ...
        echo "field is present -- new_parts_instled In HelpDesk Module --- <br>";
    }
} else {
    echo "Block Does not exits -- LBL_TICKET_INFORMATION in HelpDesk -- <br>";
}
...
$blockInstance = Vtiger_Block::getInstance('LBL_TICKET_INFORMATION', $invoiceModule);
if ($blockInstance) {
    $fieldInstance = Vtiger_Field::getInstance('invoice_number', $invoiceModule);
    if (!$fieldInstance) {
        $field = new Vtiger_Field();
        $field->name = 'invoice_number';
        $field->column = 'invoice_number';
        $field->uitype = 2;
        $field->table = $invoiceModule->basetable;
        ...
        echo "field is present -- invoice_number in HelpDesk --- <br>";
    }
} else {
    echo "Block Does not exits -- LBL_TICKET_INFORMATION in HelpDesk -- <br>";
}
...
$blockInstance = Vtiger_Block::getInstance('LBL_TICKET_INFORMATION', $invoiceModule);
if ($blockInstance) {
    $fieldInstance = Vtiger_Field::getInstance('amount_in_words', $invoiceModule);
    if (!$fieldInstance) {
        $field = new Vtiger_Field();
        $field->name = 'amount_in_words';
        $field->column = 'amount_in_words';
        $field->uitype = 19;
        $field->table = $invoiceModule->basetable;
        ...
        echo "field is present -- amount_in_words In HelpDesk Module --- <br>";
    }
} else {
    echo "Block Does not exits -- LBL_TICKET_INFORMATION -- <br>";
}
...
$emm = new VTEntityMethodManager($adb);
$result = $adb->pquery("SELECT function_name FROM com_vtiger_workflowtasks_entitymethod WHERE module_name=? AND method_name=?", array('HelpDesk', 'CalculateAmountInWords'));
if ($adb->num_rows($result) == 0) {
    $emm->addEntityMethod("HelpDesk", "CalculateAmountInWords", "modules/HelpDesk/CalculateAmountInWords.php", "CalculateAmountInWords");
} else {
    print_r("already exits --- workflow function -- CalculateAmountInWords <br> ");
}
$emm = null;

$emm = new VTEntityMethodManager($adb);
$result = $adb->pquery("SELECT function_name FROM com_vtiger_workflowtasks_entitymethod WHERE module_name=? AND method_name=?", array('HelpDesk', 'UserSpecificNumberCreation'));
if ($adb->num_rows($result) == 0) {
    $emm->addEntityMethod("HelpDesk", "UserSpecificNumberCreation", "modules/HelpDesk/UserSpecificNumberCreation.php", "UserSpecificNumberCreation");
} else {
    print_r("already exits --- workflow function -- UserSpecificNumberCreation <br> ");
}
$emm = null;

$emm = null;
$emm = new VTEntityMethodManager($adb);
$result = $adb->pquery("SELECT function_name FROM com_vtiger_workflowtasks_entitymethod WHERE module_name=? AND method_name=?", array('HelpDesk', 'HandlePUshNotiTicketAssignment'));
if ($adb->num_rows($result) == 0) {
    $emm->addEntityMethod("HelpDesk", "HandlePUshNotiTicketAssignment", "modules/HelpDesk/HandlePUshNotiTicketAssignment.php", "HandlePUshNotiTicketAssignment");
} else {
    print_r("already exits --- workflow function -- HandlePUshNotiTicketAssignment in HelpDesk Module <br> ");
}
$emm = null;