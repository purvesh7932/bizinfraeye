...
<html ng-app="portalapp">
	...
	<head>
		...
		<link rel="stylesheet" href="layouts/{portal_layout()}/skins/default/stylechanges.css" type="text/css" media="screen" />
	...
	<body ng-controller="MainController" ng-cloak>
		<nav class="navbar navbar-inverse navbar-static-top">
			...
			<div class="collapse navbar-collapse" role="navigation">
				...
				<div class="col-lg-8 col-md-8 col-sm-6 col-xs-7 portalnavbar customerportal_menu" ng-if="loginUser">
					...
					<div class="col-lg-2 col-md-2 col-sm-2 col-xs-4 portalnavbar" ng-if="loginUser">
				...
				<script type="text/ng-template" id="changePassword.template">
					...
					<form class="form-horizontal" role="form">
						...
						<div class="modal-footer">
							<button ng-if="data.oldPassword.length && data.newPassword.length && data.confirmPassword.length && (data.newPassword==data.confirmPassword) && (data.newPassword!==data.oldPassword)" class="btn btn-soft-danger" type="button" ng-click="cancel()">{{'Cancel'|translate}}</button>
							<button ng-if="data.oldPassword.length && data.newPassword.length && data.confirmPassword.length && (data.newPassword==data.confirmPassword) && (data.newPassword!==data.oldPassword)" class="btn btn-soft-success" type="submit" ng-click="save()">{{'Save'|translate}}</button>
						</div>
					</form>
				</script>
						...
						<tbody>
							...
							<tr>
								...
								<td ng-show="showMeridian">
									<button type="button" class="btn btn-soft-secondary text-center" ng-click="toggleMeridian()">{{meridian}}</button>
								</td>
							</tr>
							<tr class="text-center">
								<td>
