{strip}
				...
				<div class="row">
					<div class="col-lg-10">
						<div class="input-group" style="width:100%">
							...
							<div class="col-lg-2">
							{* {if $data.$collect_immidiately neq 1} *}
								{if $row_no eq 0}
	...
	<td>
		{if $COMMENT_EDITABLE}
			{* {if $data.$collect_immidiately eq 1} readonly {/if} *}
			<div><textarea  readonly style="pointer-events: none;background-color:#eeeeee" id="{$comment}" readonly name="{$comment}" class="lineItemCommentBox">{decode_html($data.$comment)}</textarea></div>
		{/if}
	</td>
	<td>
	{* {if $data.$collect_immidiately eq 1} readonly {/if} *}
	...
	{foreach key=LINEITEM_CUSTOM_FIELDKEY item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS}
		<td>
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '21' or $LINEITEM_CUSTOM_FIELD['uitype'] eq '2'}
				<div><textarea id="{${$fieldName}}" name="{${$fieldName}}" class="hide {$fieldName}">{decode_html($data.${$fieldName})}</textarea></div>
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '16'}
				<select style="min-width: 150px;" id="{${$fieldName}}" class="select2-container inputElement select2 inputElement picklistfield" name="{${$fieldName}}" class="inputElement picklistfield" data-fieldtype="picklist">
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '56'}
				<input class="inputElement {$fieldName} hide" id="{${$fieldName}}" name="{${$fieldName}}" style="width:15px;height:15px;" data-fieldname="{${$fieldName}}" data-fieldtype="checkbox" type="checkbox"
