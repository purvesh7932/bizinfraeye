...
<div name='editContent'>
	...
	{if $BLOCK_FIELDS|@count gt 0}
		<div class='fieldBlockContainer'>
			<div class="row">
				<div class="col-sm-6">
					<h4 class='fieldBlockHeader' style="padding:10px 0px;">{vtranslate('PARTS_FROM_SR', $MODULE)}</h4>
				</div>
			...
			<div class="lineitemTableContainer">
				<table class="table table-bordered" id="lineItemTab1">
					...
					<tr id="anorow0" class="hide lineItemCloneCopy" data-row-num="0">
						{include file="partials/LineItemsContent1.tpl"|@vtemplate_path:'ServiceOrders' row_no=0 data=[] IGNORE_UI_REGISTRATION=true}
					</tr>
					{foreach key=row_no item=data from=$RELATED_PRODUCTS_OTHER}
						<tr id="row{$row_no}" data-row-num="{$row_no}" class="lineItemRow" {if $data["entityType$row_no"] eq 'Products'}data-quantity-in-stock={$data["qtyInStock$row_no"]}{/if}>
							{include file="partials/LineItemsContent1.tpl"|@vtemplate_path:'ServiceOrders' row_no=$row_no data=$data}
						</tr>
					{/foreach}
					{* {if count($RELATED_PRODUCTS_OTHER) eq 0 and ($PRODUCT_ACTIVE eq 'true' || $SERVICE_ACTIVE eq 'true')}
						<tr id="row1" class="lineItemRow" data-row-num="1">
							{include file="partials/LineItemsContent1.tpl"|@vtemplate_path:'ServiceOrders' row_no=1 data=[] IGNORE_UI_REGISTRATION=false}
						</tr>
					{/if} *}
				</table>
			</div>
