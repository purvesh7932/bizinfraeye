Inventory_Edit_Js("RecommissioningReports_Edit_Js", {
	...
	handleMandatoryDependency: function (e) {
		...
		mandatoryKeys.forEach(element => {
			if (element != "") {
				$('#RecommissioningReports_editView_fieldName_' + element).attr('required', true);
	...
	deMandatoryDependency: function (e) {
		...
		mandatoryKeys.forEach(element => {
			if (element != "") {
				$('#RecommissioningReports_editView_fieldName_' + element).attr('required', false);
	...
	*/
	...
	optionPartsAffected: function () {
		$("#RecommissioningReports_Edit_fieldName_fail_de_parts_affected").removeClass("select2 select2-offscreen");

		$("#RecommissioningReports_Edit_fieldName_fail_de_parts_affected").addClass("hide");
		var option = $("#RecommissioningReports_Edit_fieldName_fail_de_parts_affected option");
		var data = [], temp = [];
		for (let i = 0; i < option.length; i++) {
			const myArray = option[i].value.split("_._");
			if (temp.length == 0) {
		...
		$('input[name="fail_de_parts_affected"]').select2({
			...
			var select2Value = $(e.target).val();
			const myArray = select2Value.split(",");
			$('#RecommissioningReports_Edit_fieldName_fail_de_parts_affected option:selected').removeAttr('selected');
			for (let i = 0; i < myArray.length; i++) {
				$("#RecommissioningReports_Edit_fieldName_fail_de_parts_affected option[value='" + myArray[i] + "']").attr("selected", "selected").trigger('change');;
			}
		});
		var optionsSelected = $('#RecommissioningReports_Edit_fieldName_fail_de_parts_affected').val();
		if (optionsSelected != null) {
			var damagedSelectedData = [];
			for (let i = 0; i < optionsSelected.length; i++) {
				const showText = optionsSelected[i].split('_._');
	...
	optionSystemAffected: function () {
		$("#RecommissioningReports_Edit_fieldName_fail_de_system_affected").removeClass("select2 select2-offscreen");

		$("#RecommissioningReports_Edit_fieldName_fail_de_system_affected").addClass("hide");
		var option = $("#RecommissioningReports_Edit_fieldName_fail_de_system_affected option");
		var data = [], temp = [];
		for (let i = 0; i < option.length; i++) {
			const myArray = option[i].value.split("_._");
			if (temp.length == 0) {
		...
		$('input[name="fail_de_system_affected"]').select2({
			...
			var select2Value = $(e.target).val();
			const myArray = select2Value.split(",");
			$('#RecommissioningReports_Edit_fieldName_fail_de_system_affected option:selected').removeAttr('selected');
			for (let i = 0; i < myArray.length; i++) {
				$("#RecommissioningReports_Edit_fieldName_fail_de_system_affected option[value='" + myArray[i] + "']").attr("selected", "selected").trigger('change');;
			}
		});
		var optionsSelected = $('#RecommissioningReports_Edit_fieldName_fail_de_system_affected').val();
		if (optionsSelected != null) {
			var damagedSelectedData = [];
			for (let i = 0; i < optionsSelected.length; i++) {
				const showText = optionsSelected[i].split('_._');
	...
	comparePopupHandler: function () {
		...
		$("#SubAssemblyLookUP").on('click', function () {
			var params = {};
			params.module = 'RecommissioningReports';
			params.src_module = 'RecommissioningReports';
			params.src_field = '';
			params.src_record = '';
			params.view = 'Popup';
