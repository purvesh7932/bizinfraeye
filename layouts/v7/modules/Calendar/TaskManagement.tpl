...
{strip}
	<div id="taskManagementContainer" class='fc-overlay-modal modal-content' style="height:100%;width: 98%; margin: auto;">
		<input type="hidden" name="colors" value='{json_encode($COLORS)}'>
		<div class="overlayHeader">
			{assign var=HEADER_TITLE value="Todo List"}
			...
			<div class='datacontent'>
				<div class="data-header clearfix">
					<div class="btn-group dateFilters pull-left" role="group" aria-label="...">
						<button type="button" class="btn btn-soft-primary {if $TASK_FILTERS['date'] eq "all"}active{/if}" data-filtermode="all">{vtranslate('LBL_ALL', $MODULE)}</button>
						<button type="button" class="btn btn-soft-primary {if $TASK_FILTERS['date'] eq "today"}active{/if}" data-filtermode="today">{vtranslate('LBL_TODAY', $MODULE)}</button>
						<button type="button" class="btn btn-soft-primary {if $TASK_FILTERS['date'] eq "thisweek"}active{/if}" data-filtermode="thisweek">{vtranslate('LBL_THIS_WEEK', $MODULE)}</button>
						<button type="button" class="btn btn-soft-primary dateRange dateField" data-calendar-type="range" data-filtermode="range"><i class="fa fa-calendar"></i></button>
						<button type="button" class="btn btn-soft-primary hide rangeDisplay">
					...
					<div id="taskManagementOtherFilters" class="otherFilters pull-right" style="width:550px;">
						...
						<div><button class="btn btn-soft-primary search"><span class="fa fa-search"></span></button></div>
					</div>
				</div>
					...
					{foreach item=PRIORITY from=$PRIORITIES}
						<div class="col-lg-4 contentsBlock {strtolower($PRIORITY)} ui-droppable" data-priority='{$PRIORITY}' data-page="{$PAGE}">
							<div class="{strtolower($PRIORITY)}-header" style="border-bottom: 2px solid {$COLORS[$PRIORITY]}">
								<button value='{json_encode($CLASS)}' class="{$CLASS[$PRIORITY]}"><span>{$PRIORITY}</span></button>
							</div>
							<br>
							<div class="{strtolower($PRIORITY)}-content content" data-priority='{$PRIORITY}' style="border-bottom: 1px solid {$COLORS[$PRIORITY]};padding-bottom: 10px">
								{if $USER_PRIVILEGES_MODEL->hasModuleActionPermission($MODULE_MODEL->getId(), 'CreateView')}
									<div class="input-group">
										...
										<span class="quickTask input-group-addon js-task-popover-container more cursorPointer btn btn-soft-primary" id="basic-addon1" style="border: 1px solid #ddd; padding: 0 13px;"> 
