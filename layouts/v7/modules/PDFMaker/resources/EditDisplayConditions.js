PDFMaker_Edit_Js("PDFMaker_EditDisplayConditions_Js", {
}, {
   ...
   checkDuplicateFieldsSelected: function () {
      ...
      jQuery.each(selectedFieldNames, function (i, ele) {
         ...
         if (taskType == "VTUpdateFieldsTask") {
            var fields = jQuery('[data-pdfmaker_columnname="' + fieldName + '"]').not(':hidden');
         } else {
   ...
   loadFieldSpecificUi: function (fieldSelect) {
      ...
      var taskType = jQuery('#taskType').val();
      if (taskType == "VTUpdateFieldsTask") {
         fieldValueMappingKey = fieldInfo.pdfmaker_columnname;
      }
      if (fieldValueMapping != '' && typeof fieldValueMapping[fieldValueMappingKey] != 'undefined') {
         ...
         fieldInfo.pdfmaker_valuetype = fieldValueMapping[fieldValueMappingKey]['valuetype'];
      } else {
         fieldInfo.pdfmaker_valuetype = 'rawtext';
      }
      ...
      fieldSpecificUi.filter('[name="' + fieldName + '"]').attr('data-value', 'value').attr('data-pdfmaker_columnname', fieldInfo.pdfmaker_columnname);
      fieldSpecificUi.find('[name="' + fieldName + '"]').attr('data-value', 'value').attr('data-pdfmaker_columnname', fieldInfo.pdfmaker_columnname);
   ...
   getFieldSpecificUi: function (fieldSelectElement) {
    ...
    registerEvents: function () {
        ...
        this.advanceFilterInstance = PDFMaker_AdvanceFilter_Js.getInstance(jQuery('.filterContainer',container));
        this.getPopUp();
    }
});
jQuery.fn.extend({
