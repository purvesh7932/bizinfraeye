...
jQuery.Class("Vtiger_Popup_Js",{
		...
		Vtiger_BaseList_Js.getPageRecords(params).then(
            function(data){
				...
				thisInstance.getPageCount().then(function(data){
					thisInstance.popupPageContentsContainer.find('#totalRecordsCount').text(data.numberOfRecords);
					thisInstance.popupPageContentsContainer.find('#selectAllMsgDiv').closest('div.messageContainer').removeClass('hide');
				});
	...
	doneWhenSelectedAll : function(result, eventToTrigger, excludedIds){
        var event = "post.RecordListSelectionAll.click";
        if(typeof eventToTrigger !== 'undefined'){
            event = eventToTrigger;
        }
        if(typeof event == 'function') {
            ...
            app.event.trigger(event, result, JSON.stringify(this.getPopupListSearchParams()), excludedIds);
        }
        app.helper.hidePopup();
    },
	...
	*/
    ...
    registerSelectButton : function(){
		...
		popupPageContentsContainer.on('click','button.select', function(e){
			...
			var dataUrl;
			var selectedData = thisInstance.readSelectedIds();
			let excludedIds = thisInstance.reladExcludedIds(false);
			for(var data in selectedData){
				if(typeof selectedData[data] == "object"){
					var id = selectedData[data]['id'];
					recordIds.push(id);
			...
			if(Object.keys(selectedRecordDetails).length <= 0) {
				if(selectedData == 'all'){
					thisInstance.doneWhenSelectedAll('all', "post.RecordListSelectionAll.click", excludedIds);
				}
			}else{
				if(typeof dataUrl != 'undefined'){
...
//						}
	...
	registerSelectAllClickEvent: function () {
        var self = this;
        var selectAllContainer = self.popupPageContentsContainer.find('#selectAllMsgDiv');
        selectAllContainer.click(function () {
            self.popupPageContentsContainer.find('.entryCheckBox').each(function (i, ele) {
                jQuery(ele).attr('checked', true);
            });
            self.popupPageContentsContainer.find('#selectAllInCurrentPage').attr('checked', true);
            self.writeSelectedIds("all");
			let selectionButton = jQuery('#popupContents').find('.select');
			selectionButton.removeAttr("disabled");
            selectAllContainer.closest('div.messageContainer').addClass('hide');
            self.popupPageContentsContainer.find('#deSelectAllMsgDiv').closest('div.messageContainer').removeClass('hide');
        });
    },
	registerDeselectAllClickEvent: function () {
        var self = this;
        var deselectAllContainer = self.popupPageContentsContainer.find('#deSelectAllMsgDiv');
        deselectAllContainer.click(function () {
            self.popupPageContentsContainer.find('.entryCheckBox').each(function (i, ele) {
                jQuery(ele).attr('checked', false);
            });
            self.popupPageContentsContainer.find('#selectAllInCurrentPage').attr('checked', false);
	...
	writeExcludedIds: function (excludedIds) {
        var self = this;
        var element = self.popupPageContentsContainer.find('#excludedIds');
	...
	registerEventForCheckboxChange : function(){
		...
		popupPageContentsContainer.on('click','input.entryCheckBox',function(e){
			...
			if (checkBoxJqueryObject.is(':checked')) {
				...
				var recordIds = thisInstance.getSelectedRecordIds();
				if (selectedId == 'all') {
					var element = thisInstance.popupPageContentsContainer.find('#excludedIds');
        			var excludedIds1 = element.data('excluded-ids');
					excludedIds1.push(row.data('id'));
