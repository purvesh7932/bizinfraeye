...
{assign var=IS_PACKAGE_INCLUDED value=$EXTENSION->isPackageIncludedModule()}
<tr class="installedModuleRow {if $IS_PACKAGE_INCLUDED}packageIncluded{else}packageNotIncluded{/if}" data-cfmid="1" id="{$MODULE_NAME}" {if !$IS_PACKAGE_INCLUDED}title="{vtranslate('LBL_NOT_INCLUDED_IN_PACKAGE', $QUALIFIED_MODULE)}"{/if}>
    <td><a {if $MODULE_MODEL && $MODULE_MODEL->isActive()}href="{$MODULE_MODEL->getDefaultUrl()}" target="_blank" {else} href="#{$MODULE_NAME}" {/if}>{vtranslate($EXTENSION->get('label'), $QUALIFIED_MODULE)}</a></td>
