		...
		{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name=blockIterator}
			{if $BLOCK_FIELDS|@count gt 0}
				<div class='fieldBlockContainer' data-block="{$BLOCK_LABEL}">
						...
						<tr>
							...
							{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
								...
								{if $FIELD_MODEL->isEditable() eq true}
									...
									{/if}
									<td class="fieldLabel alignMiddle" data-td="{$FIELD_MODEL->getFieldName()}">
					...
					{if $BLOCK_LABEL eq 'daadcp'}
						{include file="partials/LineItemsEdit2.tpl"|@vtemplate_path:'Equipment'}
					{/if}
				</div>
			{/if}
		{/foreach}
