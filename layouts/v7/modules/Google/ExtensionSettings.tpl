...
<div class="col-sm-12 col-xs-12 extensionContents">
	...
	<form name="settingsForm" action="index.php" method="POST" >
		...
		<div style="margin-top: 8%;">
			<div>
				<button id="saveSettings" type="submit" class="btn btn-soft-success saveButton">{vtranslate('LBL_SAVE_SETTINGS', $MODULENAME)}</button>
				{if $PARENT neq 'Settings'}
					<a type="reset" data-url="{$MODULE_MODEL->getBaseExtensionUrl($SOURCEMODULE)}" class="cancelLink navigationLink btn btn-soft-danger">{vtranslate('LBL_CANCEL', $MODULENAME)}</a>
				{/if}
			</div>
		</div>
	</form>
