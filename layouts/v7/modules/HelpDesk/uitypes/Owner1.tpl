...
{if $FIELD_MODEL->get('uitype') eq '53'}
	...
	{/if}
    <script type="text/javascript" src="libraries/jquery/select2/select3.min.js"></script>
	<select class="inputElement ownerClass" type="owner" data-fieldtype="owner" data-fieldname="{$ASSIGNED_USER_ID}" data-name="{$ASSIGNED_USER_ID}" name="{$ASSIGNED_USER_ID}" 
		...
		<optgroup label="{vtranslate('LBL_USERS')}">
			{foreach key=OWNER_ID item=OWNER_NAME from=$ALL_ACTIVEUSER_LIST}
                    <option value="{$OWNER_ID}"  data-name=' {$OWNER_NAME}'data-number='{$OWNERASSIGNEDCOUNTLIST[$OWNER_ID]} {$OWNERLEAVESTATUSLIST[$OWNER_ID]}' data-picklistvalue= '{$OWNER_NAME}' {if $FIELD_VALUE eq $OWNER_ID && $VIEW_SOURCE neq 'MASSEDIT'} selected {/if}
						{if array_key_exists($OWNER_ID, $ACCESSIBLE_USER_LIST)} data-recordaccess=true {else} data-recordaccess=false {/if}
						data-userId="{$CURRENT_USER_ID}">{$OWNER_NAME}</option>
			{/foreach}
		</optgroup>
	...
	<script>
	   function formatState (state) {
		var number=$(state.element).attr("data-number");
		if( $(state.element).attr("data-number")!=undefined && number != " "){
          var $state = $(
			state.text+' '+'<span class="badge bg-primary">'+ number+'</span>'
		  );
