{strip}
				...
				<div class="row">
					<div class="col-lg-10">
						<div class="input-group" style="width:100%">
							...
							{/if}
							<input type="hidden" id="{$igLineItemId}" name="{$igLineItemId}" value="{$data.$igLineItemId}" class="selectedModuleId lokiii"/>
	...
	<td>
		<input type="text" data-extraname="qty" id="{$qty}" style="background-color:#eeeeee!important" name="{$qty}" class="qty smallInputBox inputElement {$qty}"
			  readonly data-rule-required=true data-rule-positive=true data-rule-greater_than_zero=true value="{if !empty($data.$qty)}{$data.$qty}{else}1{/if}"
	...
	{assign var="total_excluded_qty" value="total_excluded_qty"|cat:$row_no}
	<input hidden data-extraname="total_excluded_qty" id="{$total_excluded_qty}" value="{decode_html($data.${'total_excluded_qty'})}" class="total_excluded_qty inputElement {$total_excluded_qty}"></input>
		...
		<td>
			...
			{if  $LINEITEM_CUSTOM_FIELD['uitype'] eq '5'}
				<div class="input-group inputElement" style="margin-bottom: 3px">
					<input {if $LINEITEM_CUSTOM_FIELD['editable'] eq 0} disabled style="pointer-events: none;" {/if} id="{${$fieldName}}" id="{${$fieldName}}" name="{${$fieldName}}" type="text" class="dateField form-control" data-fieldname="{${$fieldName}}" data-fieldtype="date" data-date-format="{$dateFormat}"
					...
					{if $LINEITEM_CUSTOM_FIELD['editable'] neq 0} <span class="input-group-addon"><i class="fa fa-calendar "></i></span>{/if}
				</div>
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '21' or $LINEITEM_CUSTOM_FIELD['uitype'] eq '2'}
				<div id="{$fieldName}DivCla" {if $LINEITEM_CUSTOM_FIELD['hideInitialDisplay'] eq 'true'} class="hide" {/if}>
					<textarea {if $LINEITEM_CUSTOM_FIELD['editable'] eq 0} disabled style="pointer-events: none;" {/if} id="{${$fieldName}}" name="{${$fieldName}}" class="">
						{decode_html($data.${$fieldName})}
					</textarea>
				</div>
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '7' }
				<div><input data-extraname="{$fieldName}" {if $LINEITEM_CUSTOM_FIELD['editable'] eq 0} readonly style="pointer-events: none;background-color:#eeeeee" {/if} id="{${$fieldName}}" name="{${$fieldName}}" value="{decode_html($data.${$fieldName})}" class="{$fieldName} inputElement" style="min-width: 100px;" data-rule-positive=true></input></div>
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '16'}
				<select {if $LINEITEM_CUSTOM_FIELD['editable'] eq 0} disabled style="pointer-events: none;" {/if} style="min-width: 150px;" id="{${$fieldName}}" class="select2-container inputElement select2 inputElement picklistfield {$fieldName}" name="{${$fieldName}}" data-extraname="{$fieldName}" class="inputElement picklistfield" data-fieldtype="picklist">
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '56'}
				<input {if $LINEITEM_CUSTOM_FIELD['editable'] eq 0} disabled style="pointer-events: none;" {/if} class="inputElement" id="{${$fieldName}}" name="{${$fieldName}}" style="width:15px;height:15px;" data-fieldname="{${$fieldName}}" data-fieldtype="checkbox" type="checkbox"
