{strip}
	...
	{if $QUANTITY_VIEWABLE}
		<td>
			<input disabled id="{$qty_other}" name="{$qty_other}" type="text" class="qty smallInputBox inputElement"
	...
	{foreach key=LINEITEM_CUSTOM_FIELDKEY item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS_OTHER}
		<td>
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '16'}
				<select style="min-width: 150px;" id="{${$fieldName}}" class="select2-container select2 inputElement picklistfield lineitempicklistfield {${$fieldName}}" name="{${$fieldName}}" data-fieldname="{$fieldName}" data-rownum="{$row_no}" data-fieldtype="picklist">
					<option disabled value="">{vtranslate('LBL_SELECT_OPTION','Vtiger')}</option>
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '56'}
				<input class="inputElement" data-rownum="{$row_no}" id="{${$fieldName}}" name="{${$fieldName}}" style="width:15px;height:15px;" data-fieldnameig="{$fieldName}" data-fieldtype="checkbox" type="checkbox"
