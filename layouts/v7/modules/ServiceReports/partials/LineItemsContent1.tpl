{strip}
    ...
    {assign var="comment_other" value="comment_other"|cat:$row_no}
	...
	{if $PRODUCT_EDITABLE}
		...
		<td>
			{if $COMMENT_EDITABLE}
				<div><textarea readonly style="pointer-events: none;background-color:#eeeeee" id="{$comment_other}" name="{$comment_other}" class="lineItemCommentBox">{decode_html($data.$comment_other)}</textarea></div>
			{/if}
		</td>
	{/if}
