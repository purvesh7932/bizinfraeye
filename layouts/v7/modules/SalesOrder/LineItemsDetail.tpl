                    ...
                    <tr>
                        ...
                        {if $PRODUCT_VIEWABLE}
                            <td>
                                ...
                                {/if}
                                {assign var=finalQty value=(int)$LINE_ITEM_DETAIL["final_qty$INDEX"]}
                                {assign var=createdQty value=(int)$LINE_ITEM_DETAIL["rso_created_qty$INDEX"]}
                                {assign var=creatableQTy value= $finalQty - $createdQty}
                                {if $creatableQTy gt 0 and $CAN_CREATE_RSO eq true}
                                    <h5>
                                        <a class="fieldValue" 
                                            href="index.php?module=ReturnSaleOrders&view=Edit&parent_line_itemid={$LINE_ITEM_DETAIL["igLineItemId$INDEX"]}&sale_order_id={$RECORD->get('id')}&ext_app_num_noti={$RECORD->get('po_no')}" target="_self">
                                            Create Return SalesOrder
                                        </a>
                                    </h5>
                                {/if}
                            </td>
